...
parser.add_argument('--modelname', type=str, default='GCN',  choices=['GCN', 'GCN_V1', 'GCN_V2', 'GAT','GIN', 'JK'])
...
parser.add_argument("--log", type=str, help="dir of the log file", default='nettack_di_results')
...
def multi_test():
    ...
    # no attack
    acc = single_test(adj, features, node_list)
    logger.info('Accuracy without attack: %f, # of targets: %d' % (acc, num))
    ...
    for target_node in node_list:
        ...
        cnt += acc
        num_tar += 1
        print('classification rate : %f' % (cnt/num_tar), '# of targets:', num_tar)

    logger.info('Accuracy with attack: %f, # of targets: %d' % (cnt/num_tar, num))

"""Set attention"""
attention = args.GNNGuard
print(attention)
if attention: print("Using GNNGuard")
else: print("Not using GNNGuard")
...
def single_test(adj, features, target_node):
    ...
    _, test_acc = train_dgl_model(args.modelname, 16, labels.max().item() + 1, 0.5 , \
                    features, adj, labels, idx_train, idx_val, target_node, n_epochs=200, \
...
def select_nodes(num_target = 10):
    ...
    if modelname in ['GCN_V1', 'GCN_V2']: modelname = 'GCN'
