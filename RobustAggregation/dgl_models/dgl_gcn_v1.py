...
class NodeApplyModule_V1(nn.Module):
    def __init__(self, out_feats, activation=None, bias=True):
        super(NodeApplyModule_V1, self).__init__()
        if bias:
...
class GCNLayer_V1(nn.Module):
    def __init__(self,
        ...
        self.node_update = NodeApplyModule_V1(out_feats, activation, bias)
        self.reset_parameters()
    ...
    def message_func(self, edge):
        ...
        if self.aggr == "median":
            return {'m': msg, 'self': edge.dst['h'] * edge.dst['norm'], 'input_ft': edge.src['h']}
        else:
            return {'m': msg}
...
class GCN_V1(nn.Module):
    def __init__(self,
        ...
        # input layer
        self.layers.append(GCNLayer_V1(g, in_feats, n_hidden, activation, dropout, \
                                    aggr=aggr, n_neigh_threshold=n_neigh_threshold, trim_ratio=trim_ratio, trim_compensate=trim_compensate))
        # hidden layers
        for i in range(n_layers - 1):
            self.layers.append(GCNLayer_V1(g, n_hidden, n_hidden, activation, dropout, \
                                        aggr=aggr, n_neigh_threshold=n_neigh_threshold, trim_ratio=trim_ratio, trim_compensate=trim_compensate))
        # output layer
        self.layers.append(GCNLayer_V1(g, n_hidden, n_classes, None, dropout, \
                                    aggr=aggr, n_neigh_threshold=n_neigh_threshold, trim_ratio=trim_ratio, trim_compensate=trim_compensate))
